...
describe('DIVOC crypto', function() {
  ...
  it('should Verify the json', async () => {
    ...
    expect(result.contents).to.be.not.null;
  ...
  it('should Verify w/ Production Key', async () => {
    ...
    expect(result.status).to.be.equals("verified");
    expect(result.contents).to.be.not.null;
    expect(result.issuer.displayName.en).to.be.equals("Gov of India");
  });
  ...
  it('should Unpack Verify JSON (not binary)', async () => {
    const result = await unpackAndVerify(JSON_PAYLOAD, PUBLIC_KEY_PEM);
    expect(result.status).to.be.equals("verified");
    expect(result.contents).to.eql(JSON.parse(JSON_PAYLOAD));
    expect(result.issuer).to.not.be.null;
  });
