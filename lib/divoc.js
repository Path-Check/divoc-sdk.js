...
export async function verify(signed, publicKey) {
  if (publicKey) return await verifyHack(signed, publicKey)

  const issuer = await resolveKey(signed.proof.verificationMethod)
  if (issuer) return await verifyHack(signed, issuer.didDocument);
