...
import {vaccinationContext as divocContext, 
        vaccinationContextV2 as divocContextV2} from "vaccination-context"
...
export async function verify(signed, publicKey) {
  return await verifyHack(signed, INDIA_PROD_PUBLIC_KEY_PEM) 
     ...
     ||  await verifyHack(signed, PHILIPPINES_PROD_PUBLIC_KEY_PEM)
...
export async function unpack(binary) {
    if (binary.includes("did:india") || binary.includes("did:srilanka") || binary.includes("did:philippines")) {
