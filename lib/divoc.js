...
export async function verify(signed, publicKey) {
  return await verifyHack(signed, INDIA_PROD_PUBLIC_KEY_PEM) 
     ...
     ||  await verifyHack(signed, publicKey);
}

export async function sign(certificate, privateKeyPem) {
...
export async function signAndPack(payload, privateKeyPem) {
  return await pack(await sign(payload, privateKeyPem));
}

export async function unpackAndVerify(uri, publicKey) {
  try {
    const json = await unpack(uri);
    if (await verify(json, publicKey)) {
      return json;
    }
    return undefined;
  } catch (err) {
